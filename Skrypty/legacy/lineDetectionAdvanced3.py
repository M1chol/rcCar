...
def avarageCurve(points):
    ...
    for index, line in enumerate(points):
        ...
        # lines[index]=np.stack((sorted,poly(sorted)),axis=1)
        plt.show()
    return lines
...
def AnalizeForLines(image):
    ...
    lines=cv2.HoughLinesP(img_crop, 1, np.pi / 180, 70, None, minLineLength=45, maxLineGap=20)
...
def AnalizeForCurve(image):
    ...
    lines=cv2.HoughLinesP(img_crop, 2, np.pi / 180, 10, None, minLineLength=5, maxLineGap=20)  # wykrycie lini
