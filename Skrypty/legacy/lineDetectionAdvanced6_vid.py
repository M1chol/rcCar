...
def canny(image):
    ...
    img_cann=cv2.Canny(img_blur, 150, 250)
    return img_cann
...
def AnalizeForLines(image):
    ...
    try:
        lines=cv2.HoughLinesP(img_crop, 2, np.pi / 180, 60, None, minLineLength=70, maxLineGap=10)
...
while cap.isOpened():
    _, frame=cap.read()
    i1=AnalizeForLines(frame)
    cv2.imshow("normal", i1)
    i3=analizeBirdView(frame)
