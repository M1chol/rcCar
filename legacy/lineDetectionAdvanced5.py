...
def canny(image):
    ...
    img_cann=cv2.Canny(img_blur, 200, 400)
    return img_cann
...
def avarageSlopeBird(image, lines, qual=3):
    ...
    if area:
        ...
        for pointNr in range(1,len(area)):
            odl=abs(area[pointNr][0] - carret[0])
            if odl//2<threshold:
                lane1.append(area[pointNr])
                carret=area[pointNr]
            elif odl<threshold:
                angle=np.polyfit(carret, area[pointNr], 1)[0]
                lastAngle=np.polyfit(carret, lane1[:-1], 1)[0]
                if lastAngle*(1+viablePercent) <= angle <= lastAngle*(1-viablePercent):
                    lane1.append(area[pointNr])
...
def AnalizeForLines(image):
    ...
    (T, thresh)=cv2.threshold(blurred, 190, 255, cv2.THRESH_BINARY_INV)
    img_cann=canny(thresh)  # zastosowanie metody canny
    img_crop=regionOfIntrest(img_cann, 0)  # przycięcie zdjęcia
    try:
        lines=cv2.HoughLinesP(img_crop, 2, np.pi / 180, 80, None, minLineLength=60, maxLineGap=20)
        ...
        return img_comb, img_crop, thresh
    except:
        return image, img_crop, thresh
...
def AnalizeForCurve(image):
    ...
    blurred=cv2.GaussianBlur(gray, (9, 9), 0)
    (T, thresh)=cv2.threshold(blurred, 190, 255, cv2.THRESH_BINARY_INV)
    img_cann=canny(thresh)
    img_crop=regionOfIntrest(img_cann, 1)
    try:
        lines=cv2.HoughLinesP(img_crop, 2, np.pi / 180, 30, None, minLineLength=20, maxLineGap=40)  # wykrycie lini
...
while cap.isOpened():
    _, frame=cap.read()
    i1, i2, i6 = AnalizeForLines(frame)
    cv2.imshow("normal",i1)
    cv2.imshow("normal_canny",i2)
    cv2.imshow('normal_thres', i6)
    i3, i4, i5 = AnalizeForCurve(frame)
    cv2.imshow("warped",i3)
    cv2.imshow("warped_canny",i4)
    cv2.imshow('warped_thres', i5)
    cv2.waitKey(1)
